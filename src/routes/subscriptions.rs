...
async fn insert_subscriber(pool: &PgPool, s: &NewSubscriber) -> Result<Uuid, sqlx::Error> {
...
#[tracing::instrument(
    ...
    skip(pool, sub_id, sub_token)
)]
async fn store_token(pool: &PgPool, sub_id: Uuid, sub_token: &str) -> Result<(), sqlx::Error> {
    sqlx::query!(
